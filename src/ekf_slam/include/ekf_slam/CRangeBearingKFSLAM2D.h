...
class CRangeBearingKFSLAM2D :
    public bayes::
        CKalmanFilterCapable<3 /* x y yaw */, 2 /* range yaw */, 2 /* x y */, 3 /* Ax Ay Ayaw */>
...
// typename kftype = double>
{
 public:
  ...
  void processActionObservation(
      mrpt::obs::CActionCollection::Ptr& action, mrpt::obs::CSensoryFrame::Ptr& SF);
   ...
   * data-association */
  const TDataAssocInfo& getLastDataAssociation() const { return m_last_data_association; }
 ...
 protected:
  ...
  void OnObservationJacobiansNumericGetIncrements(
      KFArray_VEH& out_veh_increments, KFArray_FEAT& out_feat_increments) const override;
  ...
  void getLandmarkIDsFromIndexInStateVector(
      std::map<unsigned int, mrpt::maps::CLandmark::TLandmarkID>& out_id2index) const
  {
