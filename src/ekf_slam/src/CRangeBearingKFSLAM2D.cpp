...
#include <ekf_slam/CRangeBearingKFSLAM2D.h>
...
/*---------------------------------------------------------------
              Constructor
  ---------------------------------------------------------------*/
CRangeBearingKFSLAM2D::CRangeBearingKFSLAM2D() : options(), m_action(), m_SF(), m_IDs(), m_SFs()
{
  reset();
}
...
/*---------------------------------------------------------------
              getCurrentRobotPose
  ---------------------------------------------------------------*/
void CRangeBearingKFSLAM2D::getCurrentRobotPose(CPosePDFGaussian& out_robotPose) const
{
  MRPT_START
...
void CRangeBearingKFSLAM2D::OnTransitionNoise(KFMatrix_VxV& Q) const
{
  ...
  if (!act3D && !act2D)
  {
    ...
    for (size_t i = 0; i < 3; i++) Q(i, i) = square(options.stds_Q_no_odo[i]);
    return;
  }
  else
  {
    ...
    size_t idx_landmark_to_predict, KFMatrix_OxV& Hx, KFMatrix_OxF& Hy) const
{
  ...
  // ---------------------------------------------------
  const kftype EXP1 = -2 * yi * y0s * cphi0 - 2 * yi * y0 + 2 * xi * y0s * sphi0 - 2 * xi * x0 -
                      2 * xi * x0s * cphi0 - 2 * yi * x0s * sphi0 + 2 * y0s * y0 * cphi0 -
                      2 * y0s * x0 * sphi0 + 2 * y0 * x0s * sphi0 + square(x0) +
                      2 * x0s * x0 * cphi0 + square(x0s) + square(y0s) + square(xi) + square(yi) +
                      square(y0);
  const kftype sqrtEXP1_1 = kftype(1) / sqrt(EXP1);

  const kftype EXP2 =
      cphi0s * xi + sphi0s * yi - sin(phis) * y0s - y0 * sphi0s - x0s * cos(phis) - x0 * cphi0s;
  const kftype EXP2sq = square(EXP2);

  const kftype EXP3 =
      -sphi0s * xi + cphi0s * yi - cos(phis) * y0s - y0 * cphi0s + x0s * sin(phis) + x0 * sphi0s;
  ...
  Hx(0, 2) = (y0s * xi * cphi0 + y0s * yi * sphi0 - y0 * y0s * sphi0 - x0 * y0s * cphi0 +
              x0s * xi * sphi0 - x0s * yi * cphi0 + y0 * x0s * cphi0 - x0s * x0 * sphi0) *
             sqrtEXP1_1;

  Hx(1, 0) = (sphi0s / (EXP2) + (EXP3) / EXP2sq * cphi0s) * EXP4;
  Hx(1, 1) = (-cphi0s / (EXP2) + (EXP3) / EXP2sq * sphi0s) * EXP4;
  Hx(1, 2) = ((-cphi0s * xi - sphi0s * yi + y0 * sphi0s + x0 * cphi0s) / (EXP2) -
              (EXP3) / EXP2sq * (-sphi0s * xi + cphi0s * yi - y0 * cphi0s + x0 * sphi0s)) *
             EXP4;
    ...
    const KFMatrix_OxO& R)
{
...

  {
    ...
    for (row = 0, itObs = obs->sensedData.begin(), itDA = data_association.begin();
         itObs != obs->sensedData.end(); ++itObs, ++itDA, ++row)
    {
      ...
      {
        mrpt::containers::bimap<CLandmark::TLandmarkID, unsigned int>::iterator itID;
  ...
  if (obs_idxs_needing_data_assoc.empty())
  {
    ...
    for (size_t idxObs = 0; idxObs < data_association.size(); idxObs++)
    {
      ...
      if (da >= 0) m_last_data_association.results.associations[idxObs] = data_association[idxObs];
    }
  ...
  else
  {
    ...
    for (size_t i = 0; i < nObsDA; i++)
    {
      ...
      for (unsigned k = 0; k < obs_size; k++) Z_obs_means(i, k) = Z[idx][k];
    }
    ...
    for (size_t q = 0; q < nPredictions; q++)
    {
      ...
      for (size_t w = 0; w < obs_size; w++)
        m_last_data_association.Y_pred_means(q, w) = all_predictions[i][w];
    ...
    if (nPredictions)
    {
      ...
      mrpt::slam::data_association_full_covariance(
          ...
          m_last_data_association.results, options.data_assoc_method, options.data_assoc_metric,
    ...
    const mrpt::config::CConfigFileBase& source, const std::string& section)
{
  ...
  std_sensor_yaw =
      DEG2RAD(source.read_float(section, "std_sensor_yaw_deg", RAD2DEG(std_sensor_yaw)));
  ...
  data_assoc_method =
      source.read_enum<TDataAssociationMethod>(section, "data_assoc_method", data_assoc_method);
  data_assoc_metric =
      source.read_enum<TDataAssociationMetric>(section, "data_assoc_metric", data_assoc_metric);
...
void CRangeBearingKFSLAM2D::OnInverseObservationModel(
    const KFArray_OBS& in_z, KFArray_FEAT& yn, KFMatrix_FxV& dyn_dxv, KFMatrix_FxO& dyn_dhn) const
{
  MRPT_START
 ...
 */
void CRangeBearingKFSLAM2D::OnNewLandmarkAddedToMap(size_t in_obsIdx, size_t in_idxNewFeat)
{
  MRPT_START
    ...
    const string& styleRobot) const
{
  ...
  for (i = 0; i < nLMs; i++)
  {
    ...
    os::fprintf(
        f, "%s", math::MATLAB_plotCovariance2D(cov, mean, stdCount, styleLandmarks).c_str());
  }
    ...
    std::vector<size_t>& out_LM_indices_to_predict) const
{
  ...
  const double max_vehicle_loc_uncertainty = 4 * std::sqrt(m_pkk(0, 0) + m_pkk(1, 1));
  ...
  for (size_t i = 0; i < prediction_means.size(); i++)
    ...
    if (prediction_means[i][0] <
            (1.5 + sensor_max_range + max_vehicle_loc_uncertainty + 4 * options.std_sensor_range) &&
        fabs(prediction_means[i][1]) <
            (20.0_deg + 0.5 * fov_yaw + max_vehicle_ang_uncertainty + 4 * options.std_sensor_yaw))
#endif
    {
